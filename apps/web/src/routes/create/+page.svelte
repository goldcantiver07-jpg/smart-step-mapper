<script lang="ts">
  ...
  import ProblemAnalyzer from "$lib/components/solve/ProblemAnalyzer.svelte";
  import SolutionView from "$lib/components/solve/SolutionView.svelte";
  import PracticePanel from "$lib/components/solve/PracticePanel.svelte";
  import type { GenerateResult, ConfirmedProblem } from "$lib/components/solve/SolutionTypes";

  const topicsQuery = createQuery(() => orpc.topics.list.queryOptions());

  // ── Mode toggle (AI Generate | Manual) — drafts reset on switch ──
  let mode = $state<"ai" | "manual">("ai");
  let aiConfirmed = $state<ConfirmedProblem | null>(null);
  let aiSolution = $state<(GenerateResult & { topicId?: string | null; topicName: string }) | null>(null);
  ...
  // FR-1: drafts are not preserved when switching modes.
  function switchMode(next: "ai" | "manual") {
    if (next === mode) return;
    mode = next;
    aiConfirmed = null;
    aiSolution = null;
...
<div class="mx-auto max-w-3xl px-4 py-6">
  <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    ...
    <!-- Mode toggle -->
    <div class="flex rounded-xl border border-neutral-800 bg-neutral-900/60 p-1" role="tablist" aria-label="Creation mode">
      <button
        role="tab"
        aria-selected={mode === "ai"}
        onclick={() => switchMode("ai")}
        class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-all {mode === 'ai'
          ? 'bg-gradient-to-r from-violet-500 to-brand-600 text-white shadow-lg shadow-brand-500/20'
      ...
      <button
        ...
        class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-all {mode === 'manual'
          ? 'bg-gradient-to-r from-violet-500 to-brand-600 text-white shadow-lg shadow-brand-500/20'
          : 'text-neutral-400 hover:text-neutral-200'}"
      >
        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  {#if mode === "ai"}
    <!-- AI Generate mode (analyzer stays mounted so fields survive back-navigation) -->
    <div class:hidden={!!aiSolution}>
      <ProblemAnalyzer onSubmit={handleGenerated} />
    </div>

    {#if aiSolution && aiConfirmed}
      <SolutionView
        solution={aiSolution}
        confirmed={aiConfirmed}
        onBack={() => (aiSolution = null)}
        onPractice={() => (practiceOpen = true)}
      />
    {/if}

    {#if aiSolution && aiConfirmed && practiceOpen}
      <PracticePanel open={practiceOpen} source={aiConfirmed} onClose={() => (practiceOpen = false)} />
    {/if}
